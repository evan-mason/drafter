...
var awem = function ($) {
    ...
    function dropdownPopup(o) {
        ...
        if (o.pwrp) {
            ...
            wrap = $(rdiv('o-pwrap', rdiv('o-pmc o-pu', se, 'tabindex="-1"'))).hide();
    ...
    function parseDate(frmt, value, baseDate, mind, maxd) {
        ...
        while (frmt) {
            ...
            if (head) {
                frmt = frmt.substring(head.length);
                var str;
    ...
    function formatDate(dfrm, date) {
        ...
        while (dfrm) {
            var i = 2;
            while (i) {
                var h = dfrm.substring(0, i);
                var pfunc = formf[h];
                if (pfunc) {
                    res += pfunc(date);
                    dfrm = dfrm.substring(h.length);
                    break;
                }
                i--;
            }
            ...
            if (!i) {
                res += dfrm.substring(0, 1);
                dfrm = dfrm.substring(1);
            }
        }
    ...
    function multiselect(o) {
        ...
        function renderSel(vals) {
            ...
            if (searchOutside) {
                ...
                //todo: skip when keepFilterOnSelect
                srctxt.val(se);
                autoWidth(srctxt);
    ...
    function dropmenu(dmo) {
        ...
        var onItemClick = dmo.click || function (e) {
            ...
            //todo: move up to C#, use getSrcTerm that includes both menu search txt and outisde searchtxt
        ...
        function renderMenu() {
            ...
            var rs = getItems();
            //var count = renderList(rs, getSrcTerm()); //todo: keepFilterOnSelect

            var count = renderList(rs);

            //todo: when keepFilter don't hide because whe keep the filter
